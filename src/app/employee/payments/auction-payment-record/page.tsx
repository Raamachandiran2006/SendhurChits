...
export default function EmployeeAuctionPaymentRecordPage() {
  ...
  useEffect(() => {
    const fetchAuctionRelatedTransactions = async () => {
      ...
      try {
        ...
        setFilteredAuctionTransactions(combinedTransactions); 
  ...
  const handleDownloadPdf = () => {
    ...
    filteredAuctionTransactions.forEach((tx, index) => {
      const txData = [index + 1, tx.type, formatDateSafe(tx.dateTime), tx.fromParty, tx.toParty, formatCurrencyPdf(tx.amount), tx.mode || "N/A", tx.remarks || "N/A", tx.virtualTransactionId || "N/A"];
      tableRows.push(txData);
    });
    const filterLabel = { all: "All Time", last7Days: "Last 7 Days", last10Days: "Last 10 Days", last30Days: "Last 30 Days" };
    doc.setFontSize(18); doc.text(`Auction Payment Records (Employee View)`, 14, 15);
    ...
    autoTable(doc, {
      ...
      columnStyles: { 0: { cellWidth: 8 }, 1: { cellWidth: 15 }, 2: { cellWidth: 25 }, 3: { cellWidth: 'auto' }, 4: { cellWidth: 'auto' }, 5: { cellWidth: 20, halign: 'right' }, 6: { cellWidth: 15 }, 7: { cellWidth: 'auto' }, 8: { cellWidth: 18 } },
    });
    doc.save(`employee_auction_payment_records_${selectedFilter}.pdf`);
  };

  if (loading) {
    return (
      <div className="container mx-auto py-8 text-center">
        <Loader2 className="mx-auto h-12 w-12 animate-spin text-primary" /><p className="mt-4 text-lg text-foreground">Loading auction payment records...</p>
      </div>
    );
  }
    ...
    return (
      <div className="container mx-auto py-8 text-center">
        <Card className="max-w-md mx-auto shadow-lg">
          <CardHeader><CardTitle className="text-destructive flex items-center justify-center"><AlertTriangle className="mr-2 h-6 w-6" /> Error Loading Data</CardTitle></CardHeader>
          <CardContent><p className="text-muted-foreground whitespace-pre-line">{error}</p><Button onClick={() => window.location.reload()} className="mt-6">Try Again</Button></CardContent>
        </Card>
      </div>
    );
  }
    ...
    <div className="container mx-auto py-8">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
        <div className="flex items-center gap-3">
            ...
            <div>
                <h1 className="text-3xl font-bold text-foreground">Auction Payment Records (Employee)</h1>
                <p className="text-muted-foreground">Combined view of payments sent and received.</p>
            </div>
        </div>
        <div className="flex items-center gap-2 mt-4 sm:mt-0">
          ...
          <Button variant="outline" size="sm" onClick={handleDownloadPdf} disabled={filteredAuctionTransactions.length === 0}><Download className="mr-2 h-4 w-4" /> Download PDF</Button>
          <Button variant="outline" asChild size="sm"><Link href="/employee/payments"><ArrowLeft className="mr-2 h-4 w-4" />Back</Link></Button>
        </div>
      </div>
      <Card className="shadow-lg">
        <CardHeader>
          <div className="flex items-center gap-2"><ReceiptText className="h-6 w-6 text-primary" /><CardTitle>Transaction Log</CardTitle></div>
          <CardDescription>Shows payments made by the company (Sent) and payments received from users (Received). {error && <span className="text-destructive text-xs block mt-1">Note: Some data may have failed to load.</span>}</CardDescription>
        </CardHeader>
        <CardContent>
          {filteredAuctionTransactions.length === 0 && !error ? (<p className="text-muted-foreground text-center py-10">No auction-related payment records found {selectedFilter !== 'all' ? `for the selected period` : ''}.</p>) : (
            <div className="overflow-x-auto rounded-md border">
              <Table><TableHeader><TableRow><TableHead>S.No</TableHead><TableHead>Type</TableHead><TableHead>Date & Time</TableHead><TableHead>From</TableHead><TableHead>To</TableHead><TableHead className="text-right">Amount (₹)</TableHead><TableHead>Mode</TableHead><TableHead>Remarks</TableHead></TableRow></TableHeader>
                <TableBody>
                  {filteredAuctionTransactions.map((tx, index) => {
                    const transactionKey = tx.id + tx.originalSource; const isExpanded = expandedRows[transactionKey];
                    return (<React.Fragment key={transactionKey}>
                        <TableRow>
                          ...
                          <TableCell><span className={cn("font-semibold px-2 py-1 rounded-full text-xs", tx.type === "Sent" ? "bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300" : "bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300")}>{tx.type}</span></TableCell>
                          <TableCell>{formatDateSafe(tx.dateTime)}</TableCell><TableCell className="max-w-xs truncate">{tx.fromParty}</TableCell><TableCell className="max-w-xs truncate">{tx.toParty}</TableCell>
                          <TableCell className="text-right font-mono">{formatCurrency(tx.amount)}</TableCell><TableCell>{tx.mode || "N/A"}</TableCell><TableCell className="max-w-xs truncate">{tx.remarks || "N/A"}</TableCell>
                        </TableRow>
                      </React.Fragment>);
                  })}
                </TableBody>
