...
export default function CreditManagementPage() {
      ...
      try {
        ...
        const fetchedHistory = querySnapshot.docs.map(doc => {
          ...
          return {
            id: doc.id,
            ...data,
            filterDate: data.recordedAt ? data.recordedAt.toDate() : (data.paymentDate ? parseISO(data.paymentDate) : new Date(0)),
          } as CreditRecord & { filterDate: Date };
        });
  ...
  useEffect(() => {
    const applyFilter = () => {
      if (selectedFilter === "all") {
        setFilteredCreditHistory(rawCreditHistory);
        return;
      }
      const now = new Date();
      let startDate: Date;
  ...
  const handleDownloadPdf = () => {
    ...
    filteredCreditHistory.forEach((record, index) => {
      const recordData = [
        index + 1,
        record.fromName,
        record.creditNumber || "N/A",
        formatDateSafe(record.paymentDate),
    ...
    const filterLabel: Record<CreditFilterType, string> = {
      all: "All Time",
      today: "Today",
    ...
    doc.setFontSize(18);
    doc.text(`Credit Transaction History`, 14, 15);
    doc.setFontSize(12);
    ...
    doc.save(`credit_transaction_history_${selectedFilter}.pdf`);
  };
    ...
    <div className="container mx-auto py-8">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-8">
        ...
        <div className="flex items-center gap-2 mt-4 sm:mt-0">
          ...
          <Button variant="outline" size="sm" onClick={handleDownloadPdf} disabled={filteredCreditHistory.length === 0}>
      ...
      <Card className="shadow-xl">
        <CardHeader>
          ...
          <CardDescription>A chronological record of all credit transactions. Filtered by: {selectedFilter}</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            <div className="flex justify-center items-center py-10">
              <Loader2 className="h-12 w-12 animate-spin text-primary" />
              <p className="ml-4 text-lg text-foreground">Loading credit history...</p>
            </div>
          ) : filteredCreditHistory.length === 0 ? (
            <div className="text-center py-10">
              <p className="text-muted-foreground">No credit records found {selectedFilter !== 'all' ? `for the selected period` : ''}. Click "Add Credit Record" to add one.</p>
            </div>
          ) : (
                  ...
                  {filteredCreditHistory.map((record, index) => {
                    ...
                    <React.Fragment key={record.id}>
                      <TableRow>
                        ...
                        <TableCell>{record.creditNumber || "N/A"}</TableCell>
