...
import { Loader2, Briefcase, PlusCircle, ArrowLeft } from "lucide-react";
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        ...
        <p className="mt-4 text-lg text-foreground">Loading employees...</p>
      </div>
    );
  }
    ...
    <div className="container mx-auto py-8">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">View All Employees</h1>
            <p className="text-muted-foreground">Browse and manage employee records.</p>
          </div>
        </div>
      ...
      <Card className="shadow-xl">
        <CardHeader>
          {/* Optional: <CardTitle>Employee List</CardTitle> <CardDescription>...</CardDescription> */}
        </CardHeader>
        <CardContent>
          {employees.length === 0 ? (
            <div className="text-center py-10">
              <p className="text-muted-foreground">No employees found. Click "Add New Employee" to add one.</p>
            </div>
          ) : (
                ...
                <TableBody>
                  {employees.map((employee) => (
                    ...
                    >
                      ...
                      <TableCell>{formatDateSafe(employee.joiningDate)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
